...
# --- Superuser config (from env/.env via decouple) ---
...
# --- Guest user config ---
guest_username = config('DJANGO_GUEST_USERNAME', default='guest')
guest_email = config('DJANGO_GUEST_EMAIL', default='guest@allerjoy.com')
guest_password = config('DJANGO_GUEST_PASSWORD', default='guestpassword')
...
if not User.objects.filter(username=guest_username).exists():
    ...
    guest_user = User.objects.create_user(username=guest_username, email=guest_email, password=guest_password)
    guest_user.is_staff = False
    guest_user.is_superuser = False
    guest_user.save()
    UserProfile.objects.create(user=guest_user)

...
def send_user_email(user, subject, template_name, link_name, link_value, to_email=None):
    from_email = config("DEFAULT_FROM_EMAIL")
    site_url = config("FRONTEND_URL", "http://localhost:8000")
    context = {
        'user': user,
    ...
    msg = EmailMultiAlternatives(
        ...
        text_content,
        from_email,
        [to_email or user.email],  # standard user.email else to_email
    )
